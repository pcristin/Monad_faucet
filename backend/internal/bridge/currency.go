...
func calculateMonAmount(depositAmount *big.Int, swapRatio *big.Int, currencyType blockchain.CurrencyType) *big.Int {
	if currencyType == blockchain.CurrencyUSDC || currencyType == blockchain.CurrencyUSDT {
		...
		monUsdRatio := blockchain.GetMonUsdRatio()
		depositEthFloat := new(big.Float).Quo(new(big.Float).SetInt(depositAmount), new(big.Float).SetInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)))
		...
		humanReadableMon := new(big.Float).Quo(new(big.Float).SetInt(monWeiInt), new(big.Float).SetInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil))).Text('f', 6)
